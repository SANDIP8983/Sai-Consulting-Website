...
class UpdateOwnProfileRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'username' => ['required', 'string', 'min:3', 'max:50', 'regex:/\A[a-z0-9._-]+\z/', Rule::unique('users')->ignore($this->user())],
            'email' => ['nullable', 'email', 'max:255', Rule::unique('users')->ignore($this->user())],
            'mobile' => ['required', 'string', 'regex:/\A[6-9][0-9]{9}\z/', Rule::unique('users')->ignore($this->user())],
        ];
    }
