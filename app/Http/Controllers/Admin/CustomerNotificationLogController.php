...
class CustomerNotificationLogController extends Controller
{
    public function __invoke(Request $request): View
    {
        ...
        $deliveries = CustomerNotificationDelivery::query()->with(['event.customerRequest:id,reference_no', 'event.appointment:id,reference_no'])->latest()
            ->when($filters['q'] ?? null, fn ($q, $value) => $q->whereHas('event', fn ($event) => $event
                ->whereHas('customerRequest', fn ($r) => $r->where('reference_no', 'like', "%{$value}%"))
                ->orWhereHas('appointment', fn ($a) => $a->where('reference_no', 'like', "%{$value}%"))))
