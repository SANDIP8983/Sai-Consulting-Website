...
class DispatchManagementService
{
    ...
    public function eligibility(CustomerRequest $request): array
    {
        ...
        if (! $requiresDispatch) $reasons[] = 'Dispatch is not required for this service.';
        if (! in_array($request->status, ['completed', 'dispatched', 'delivered'], true)) $reasons[] = 'Complete the case processing before adding dispatch or delivery details.';
        if (! $request->file_number) $reasons[] = 'A file number is required before dispatch.';
        if ($requiresPayment && $request->payment_status !== 'received') $reasons[] = 'Payment Pending: payment must be confirmed before dispatch.';
        ...
        return ['eligible' => $reasons === [], 'payment_pending' => $requiresPayment && $request->payment_status !== 'received', 'reasons' => $reasons];
    }
    ...
    public function closeEligibility(CustomerRequest $request): array
    {
        ...
        if (! $dispatches->contains(fn ($dispatch) => in_array($dispatch->dispatch_status, ['delivered', 'collected'], true))) $reasons[] = 'At least one dispatch must be Delivered or Collected.';
        if ($dispatches->contains(fn ($dispatch) => in_array($dispatch->dispatch_status, ['prepared', 'dispatched', 'in_transit', 'not_dispatched'], true))) $reasons[] = 'All dispatch records must be resolved before closing the case.';
        if ($dispatches->contains(fn ($dispatch) => $dispatch->dispatch_status === 'failed_returned')) $reasons[] = 'Failed / Returned dispatch records must be resolved before closing the case.';
    ...
    public function create(CustomerRequest $request, array $attributes, User $user, ?UploadedFile $proof = null): RequestDispatch
    {
        ...
        try {
            return DB::transaction(function () use ($request, $attributes, $user, $proof, &$storedPath): RequestDispatch {
                ...
                if ($locked->status === 'closed') throw ValidationException::withMessages(['dispatch' => 'Closed cases must be reopened before adding dispatch records.']);
                $status = $attributes['dispatch_status'] ?? 'prepared';
                if (! in_array($status, ['prepared', 'dispatched', 'collected'], true)) throw ValidationException::withMessages(['dispatch_status' => 'A new dispatch must start as Prepared, Dispatched, or Collected.']);
                ...
                if ($proof) $this->storeProof($locked, $dispatch, $proof, $attributes['proof_type'], $user, $storedPath);
    ...
    public function update(CustomerRequest $request, RequestDispatch $dispatch, array $attributes, User $user): RequestDispatch
    {
        return DB::transaction(function () use ($request, $dispatch, $attributes, $user): RequestDispatch {
            ...
            $this->assertEligible($locked);
            if (in_array($locked->status, ['closed'], true)) throw ValidationException::withMessages(['dispatch' => 'Closed cases are read-only. Reopen the case first.']);
            $row = RequestDispatch::query()->lockForUpdate()->findOrFail($dispatch->id);
            if (in_array($row->dispatch_status, ['delivered', 'collected'], true)) throw ValidationException::withMessages(['dispatch' => 'Delivered or Collected records are read-only. Use the audited reopen action.']);
            $old = $row->toArray();
    ...
    public function transition(CustomerRequest $request, RequestDispatch $dispatch, string $to, array $attributes, User $user): RequestDispatch
    {
        return DB::transaction(function () use ($request, $dispatch, $to, $attributes, $user): RequestDispatch {
            ...
            $this->assertOwned($locked, $dispatch);
            $this->assertEligible($locked);
            if ($locked->status === 'closed') throw ValidationException::withMessages(['dispatch' => 'Closed cases are read-only. Reopen the case first.']);
            $row = RequestDispatch::query()->lockForUpdate()->findOrFail($dispatch->id);
            $from = $row->dispatch_status;
            if (! in_array($to, self::TRANSITIONS[$from] ?? [], true)) throw ValidationException::withMessages(['dispatch_status' => 'This dispatch-status transition is not allowed.']);
            $details = [...$row->toArray(), ...$attributes];
            $this->validateDetails($details, $to);
            $changes = ['dispatch_status' => $to, 'updated_by' => $user->id];
            if ($to === 'delivered') $changes['delivered_at'] = $attributes['delivered_at'];
            if ($to === 'collected') $changes['collected_at'] = $attributes['collected_at'];
            if ($to === 'failed_returned') $changes['failure_reason'] = $attributes['reason'];
            if ($to === 'cancelled') $changes['cancellation_reason'] = $attributes['reason'];
    ...
    public function reopenDispatch(CustomerRequest $request, RequestDispatch $dispatch, string $reason, User $user): RequestDispatch
    {
        return DB::transaction(function () use ($request, $dispatch, $reason, $user): RequestDispatch {
            $locked = CustomerRequest::query()->lockForUpdate()->findOrFail($request->id);
            $this->assertOwned($locked, $dispatch);
            if ($locked->status === 'closed') throw ValidationException::withMessages(['dispatch' => 'Reopen the closed case before reopening a dispatch record.']);
            $row = RequestDispatch::query()->lockForUpdate()->findOrFail($dispatch->id);
            if (! in_array($row->dispatch_status, ['delivered', 'collected'], true)) throw ValidationException::withMessages(['dispatch_status' => 'Only Delivered or Collected records can be reopened.']);
            $from = $row->dispatch_status;
            ...
            if ($locked->status === 'delivered') $this->setRequestStatus($locked, 'dispatched', 'Delivery record reopened.', false, $user);
            return $row->refresh();
        });
    }
    ...
    public function uploadProof(CustomerRequest $request, RequestDispatch $dispatch, UploadedFile $proof, string $type, User $user): RequestDispatchProof
    {
        ...
        try {
            return DB::transaction(function () use ($request, $dispatch, $proof, $type, $user, &$storedPath): RequestDispatchProof {
                $locked = CustomerRequest::query()->lockForUpdate()->findOrFail($request->id);
                $this->assertOwned($locked, $dispatch);
                if ($locked->status === 'closed') throw ValidationException::withMessages(['proof' => 'Closed cases are read-only.']);
    ...
    public function close(CustomerRequest $request, array $attributes, User $user): void
    {
        DB::transaction(function () use ($request, $attributes, $user): void {
            ...
            if (! $eligibility['eligible']) throw ValidationException::withMessages(['closure' => implode(' ', $eligibility['reasons'])]);
            $from = $locked->status;
    ...
    public function reopenCase(CustomerRequest $request, string $reason, User $user): void
    {
        DB::transaction(function () use ($request, $reason, $user): void {
            $locked = CustomerRequest::query()->lockForUpdate()->findOrFail($request->id);
            if ($locked->status !== 'closed') throw ValidationException::withMessages(['closure' => 'Only a Closed case can be reopened.']);
    ...
    private function assertEligible(CustomerRequest $request): void
    {
        ...
        if (! $eligibility['eligible']) throw ValidationException::withMessages(['dispatch' => implode(' ', $eligibility['reasons'])]);
    }

    private function assertOwned(CustomerRequest $request, RequestDispatch $dispatch): void
    {
        if ($dispatch->request_id !== $request->id) abort(404);
    }

    private function validateDetails(array $attributes, string $status): void
    {
        $method = $attributes['dispatch_method'];
        if ($method === 'whatsapp' && blank($attributes['recipient_mobile'] ?? null)) throw ValidationException::withMessages(['recipient_mobile' => 'Recipient mobile is required for WhatsApp delivery.']);
        if ($method === 'email' && blank($attributes['recipient_email'] ?? null)) throw ValidationException::withMessages(['recipient_email' => 'Recipient email is required for Email delivery.']);
        if (in_array($method, ['courier', 'speed_post', 'rpad'], true) && in_array($status, ['dispatched', 'in_transit', 'delivered'], true)) {
            if (blank($attributes['delivery_address'] ?? null)) throw ValidationException::withMessages(['delivery_address' => 'Postal / delivery address is required for this method.']);
            if (blank($attributes['carrier_name'] ?? null)) throw ValidationException::withMessages(['carrier_name' => 'Courier / postal service name is required for this method.']);
            if (blank($attributes['tracking_number'] ?? null)) throw ValidationException::withMessages(['tracking_number' => 'Tracking / consignment number is required before dispatch.']);
        }
        if ($method === 'office_collection' && $status === 'collected') {
            if (blank($attributes['recipient_name'] ?? null)) throw ValidationException::withMessages(['recipient_name' => 'Collected-by name is required.']);
            if (blank($attributes['collected_at'] ?? null)) throw ValidationException::withMessages(['collected_at' => 'Collection date and time is required.']);
        }
        if ($status === 'collected' && $method !== 'office_collection') throw ValidationException::withMessages(['dispatch_status' => 'Collected status is available only for Office Collection.']);
        if ($method === 'hand_delivery' && blank($attributes['recipient_name'] ?? null)) throw ValidationException::withMessages(['recipient_name' => 'Recipient name is required for Hand Delivery.']);
        if ($status === 'delivered' && blank($attributes['delivered_at'] ?? null)) throw ValidationException::withMessages(['delivered_at' => 'Delivery date and time is required.']);
        if ($method === 'other' && (blank($attributes['method_description'] ?? null) || blank($attributes['customer_remark'] ?? null))) throw ValidationException::withMessages(['method_description' => 'Other method description and customer-visible remark are required.']);
        if (in_array($status, ['failed_returned', 'cancelled'], true) && blank($attributes['reason'] ?? null)) throw ValidationException::withMessages(['reason' => 'A reason is required for Failed / Returned or Cancelled dispatch.']);
    }

    private function syncRequestStatus(CustomerRequest $request, string $status, User $user, ?string $remark): void
    {
        if (in_array($status, ['dispatched', 'in_transit'], true) && $request->status === 'completed') $this->setRequestStatus($request, 'dispatched', $remark ?: 'Documents dispatched.', true, $user);
        if (in_array($status, ['delivered', 'collected'], true) && in_array($request->status, ['completed', 'dispatched'], true)) $this->setRequestStatus($request, 'delivered', $remark ?: 'Documents delivered or collected.', true, $user);
    }
    ...
    private function storeProof(CustomerRequest $request, RequestDispatch $dispatch, UploadedFile $proof, string $type, User $user, ?string &$storedPath): RequestDispatchProof
    {
        ...
        if ($storedPath === false) throw new \RuntimeException('Dispatch proof could not be stored.');
