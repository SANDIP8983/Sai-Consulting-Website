...
<article {{ $attributes->class(['service-card premium-card w-100', 'service-card-compact' => $compact]) }} aria-labelledby="service-{{ $service->id }}">
    ...
    @if($service->short_description || $service->description_en || $service->description_gu || $service->description)<p>{{ \Illuminate\Support\Str::limit($service->short_description ?: ($service->description_en ?: $service->description_gu ?: $service->description), $compact ? 120 : 180) }}</p>@endif
    <div class="service-meta">
        @if($service->processing_time_label || $service->estimated_days)<span><i class="bi bi-clock" aria-hidden="true"></i> {{ $service->processing_time_label ?: $service->estimated_days.' days' }}</span>@endif
    ...
    <div class="service-card-actions">
        ...
        @if($service->available_online)<a class="btn btn-primary rounded-pill" href="{{ route('request.create', ['service' => $service->id]) }}">Apply Online</a>@endif
    </div>
</article>
