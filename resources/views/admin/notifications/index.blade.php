...
<div class="card border-0 shadow-sm">
    <div class="table-responsive" tabindex="0" role="region" aria-label="Customer notification deliveries">
        <table class="table align-middle mb-0">
            <thead><tr><th scope="col">Reference</th><th scope="col">Type</th><th scope="col">Milestone</th><th scope="col">Channel</th><th scope="col">Recipient</th><th scope="col">Status</th><th scope="col">Time</th><th scope="col">Failure</th></tr></thead>
            <tbody>@forelse($deliveries as $d)<tr><td>{{ $d->event->appointment?->reference_no ?? $d->event->customerRequest?->reference_no ?? '—' }}</td><td>{{ $d->event->appointment_id ? 'Appointment' : 'Customer Request' }}</td><td>{{ $d->event->milestone->label() }}</td><td>{{ str($d->channel)->title() }}</td><td>{{ $d->recipient_masked ?: '—' }}</td><td><span class="badge text-bg-{{ ['sent'=>'success','failed'=>'danger','skipped'=>'secondary'][$d->status] ?? 'warning' }}">{{ str($d->status)->title() }}</span></td><td>{{ \App\Support\IndiaDateTime::format($d->sent_at ?: $d->created_at) }} IST</td><td>{{ $d->failure_category ? str($d->failure_category)->replace('_',' ')->title() : '—' }}</td></tr>@empty<tr><td colspan="8" class="text-center text-muted py-4">No notification deliveries found.</td></tr>@endforelse</tbody>
        </table>
    </div>
