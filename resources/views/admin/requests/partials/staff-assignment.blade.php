<section class="border rounded p-3 mt-4" id="staff-assignment">
    ...
    @endcan
    @if($customerRequest->assignmentHistory->isNotEmpty())<div class="table-responsive mt-3"><table class="table table-sm mb-0"><thead><tr><th>Assigned At</th><th>Previous</th><th>Assigned Staff</th><th>Assigned By</th></tr></thead><tbody>@foreach($customerRequest->assignmentHistory as $history)<tr><td>{{ \App\Support\IndiaDateTime::format($history->assigned_at) }} IST</td><td>{{ $history->previousAssignee?->name ?? 'Unassigned' }}</td><td>{{ $history->assignee?->name ?? 'Unavailable' }}</td><td>{{ $history->assignedBy?->name ?? 'Unavailable' }}</td></tr>@endforeach</tbody></table></div>@endif
</section>
