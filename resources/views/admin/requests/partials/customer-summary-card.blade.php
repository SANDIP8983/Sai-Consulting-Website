...
@foreach([
    ...
    ['Submission Date',\App\Support\IndiaDateTime::format($customerRequest->created_at).' IST'],['Request Source',$customerRequest->isOffline()?'Offline':'Online'],
...
@if($customerRequest->contactChangeHistory->isNotEmpty())<div class="mt-4"><h4 class="h6">Contact Change History</h4><div class="table-responsive"><table class="table table-sm"><thead><tr><th>Changed At</th><th>Fields</th><th>Previous</th><th>Updated</th><th>Changed By</th></tr></thead><tbody>@foreach($customerRequest->contactChangeHistory as $change)<tr><td>{{ \App\Support\IndiaDateTime::format($change->changed_at) }} IST</td><td>{{ collect($change->changed_fields)->map(fn($field)=>str($field)->headline())->implode(', ') }}</td><td>@foreach($change->masked_old_values as $field=>$value)<div>{{ str($field)->headline() }}: {{ $value ?: 'Not provided' }}</div>@endforeach</td><td>@foreach($change->masked_new_values as $field=>$value)<div>{{ str($field)->headline() }}: {{ $value ?: 'Not provided' }}</div>@endforeach</td><td>{{ $change->changedBy->name }}</td></tr>@endforeach</tbody></table></div></div>@endif
@endcan
