...
<div class="row g-4"><div class="col-md-6"><div class="border rounded p-3 h-100"><h2 class="h5">Selected Services</h2><ul>@foreach($submission['services'] ?? [] as $service)<li><strong>{{ $service['name_gu'] }}</strong><small class="d-block">{{ $service['name_en'] }}</small></li>@endforeach</ul></div></div><div class="col-md-6"><div class="border rounded p-3 h-100"><h2 class="h5">Current Status</h2><span class="badge text-bg-primary">{{ str($submission['status'] ?? 'received')->headline() }}</span>@if($submission['estimated_days'])<p class="mt-3">Approximately {{ $submission['estimated_days'] }} working day(s)</p><p class="small text-muted">દર્શાવેલ સમય જરૂરી માહિતી અને દસ્તાવેજો ઉપલબ્ધ થયા પછીનો અંદાજિત કાર્ય સમય છે. કેસની પરિસ્થિતિ મુજબ સમયમાં ફેરફાર થઈ શકે છે.</p>@endif</div></div></div>
<div class="alert alert-info mt-4">અમારી ટીમ અરજીની ચકાસણી કર્યા બાદ તમારો સંપર્ક કરશે.<br><strong>Reference Number સાચવી રાખો.</strong><hr>Our team will contact you after reviewing your request.</div>
