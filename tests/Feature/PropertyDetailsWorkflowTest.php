...
class PropertyDetailsWorkflowTest extends TestCase
{
    ...
    {
        $service=$this->service(true); $payload=$this->payload($service);
    ...
    {
        $online=$this->service(false); $payload=$this->payload($online)+['declaration'=>'1'];
        $this->post(route('request.store'),$payload)->assertRedirect(route('request.success'));
        $offline=$this->service(false); $payload=$this->payload($offline);
