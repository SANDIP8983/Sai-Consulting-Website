...
class RequestRegistrationManagementTest extends TestCase
{
    ...
    public function test_admin_can_record_registration_and_post_registration_information(): void
    {
        ...
        $p=$request->processing->fresh(); $this->assertSame('REG-100',$p->registration_number); $this->assertTrue($p->registration_number_public); $this->assertSame('received',$p->certified_copy_status);
    }

    public function test_registered_scan_uses_private_request_document_storage(): void
    {
        Storage::fake('local'); $admin=User::factory()->create(); $request=$this->request(); app(FileDocumentProcessingService::class)->open($request, [], $admin);
        ...
        $document=$request->documents()->sole(); Storage::disk('local')->assertExists($document->file_path); $this->assertSame('admin',$document->source); $this->assertSame($document->id,$request->processing->fresh()->registered_document_id);
    }

    public function test_non_registration_service_hides_registration_sections(): void
    {
        $admin=User::factory()->create(); $request=$this->request(false); app(FileDocumentProcessingService::class)->open($request, [], $admin);
