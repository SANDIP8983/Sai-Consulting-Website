...
class DispatchManagementTest extends TestCase
{
    ...
    {
        $admin=User::factory()->create(); $request=$this->request(['status'=>'in_progress']);
    ...
    public function test_courier_requires_address_carrier_and_tracking_before_dispatched(): void
    {
        ...
        foreach(['delivery_address','carrier_name','tracking_number'] as $field){$request=$this->request();$this->actingAs($admin)->post(route('admin.requests.dispatches.store',$request),$this->payload([$field=>null]))->assertSessionHasErrors($field);}
    }
    ...
    public function test_delivered_and_collected_update_request_without_closing_it(): void
    {
        ...
        $this->assertSame('delivered',$request->fresh()->status); $this->assertNull($request->fresh()->closed_at);
    }
    ...
    {
        $admin=User::factory()->create(); $request=$this->delivered($admin);
        $this->actingAs($admin)->patch(route('admin.requests.closure.close',$request),$this->closure())->assertSessionHasNoErrors();
        $this->assertSame('closed',$request->fresh()->status); $this->assertNotNull($request->fresh()->closed_at);
        $dispatch=$request->dispatches()->first();
    ...
    {
        $admin=User::factory()->create(); $request=$this->request();
        $this->actingAs($admin)->post(route('admin.requests.dispatches.store',$request),$this->payload())->assertSessionHasNoErrors(); $dispatch=$request->dispatches()->first();
    ...
    {
        Storage::fake('local'); $admin=User::factory()->create(['name'=>'Private Dispatch Admin']); $request=$this->request();
        ...
        $this->actingAs($admin)->post(route('admin.requests.dispatches.store',$request),$data)->assertSessionHasNoErrors();
        $proof=$request->dispatches()->first()->proofs()->first(); Storage::disk('local')->assertExists($proof->file_path);
    ...
    {
        $admin=User::factory()->create(); $request=$this->request(['status'=>'dispatched']);
    ...
    {
        $admin=User::factory()->create(); $request=$this->request(); $other=$this->request();
        $dispatch=$request->dispatches()->create([...$this->payload(['dispatch_status'=>'prepared']),'performed_by'=>$admin->id]);
    ...
    {
        $request=$this->request(); $this->actingAs($admin)->post(route('admin.requests.dispatches.store',$request),$this->payload())->assertSessionHasNoErrors(); $dispatch=$request->dispatches()->first();
        $this->actingAs($admin)->patch(route('admin.requests.dispatches.status',[$request,$dispatch]),['dispatch_status'=>'delivered','delivered_at'=>now()->format('Y-m-d H:i:s')])->assertSessionHasNoErrors(); return $request->fresh();
    }

    private function closure(): array { return ['closure_date'=>now()->format('Y-m-d H:i:s'),'customer_remark'=>'Case finished','internal_note'=>'Verified closure','confirmed'=>'1']; }
    private function payload(array $attributes=[]): array { return ['dispatch_status'=>'dispatched','dispatch_method'=>'courier','dispatch_date'=>now()->subMinute()->format('Y-m-d H:i:s'),'document_description'=>'Final document package','recipient_name'=>'Customer','recipient_mobile'=>'9999999999','recipient_email'=>'customer@example.com','delivery_address'=>'Patan, Gujarat','tracking_number'=>'TRACK-1001','carrier_name'=>'Trusted Courier',...$attributes]; }
    private function request(array $attributes=[]): CustomerRequest
    {
        ...
        return CustomerRequest::query()->create(['reference_no'=>'SC/2026/'.fake()->unique()->numerify('######'),'file_number'=>'SC/2026/F'.fake()->unique()->numerify('######'),'request_origin'=>'online','service_id'=>$service->id,'name'=>'Dispatch Customer','mobile'=>'9999999999','email'=>'customer@example.com','address'=>'Patan, Gujarat','status'=>'completed','payment_status'=>'received','completed_at'=>now(),'last_status_changed_at'=>now(),...$attributes]);
    }
}
